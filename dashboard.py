...
# Campaign does not have date column, skip conversion
...
total_new_customers = daily['new customers'].sum()
avg_cpa = total_spend / total_new_customers if total_new_customers != 0 else 0
...
st.sidebar.markdown(f"**Top Platform:** {top_platform} (ROAS {top_platform_roas:.2f})")
st.sidebar.markdown(f"**Underperforming Platform:** {bottom_platform} (ROAS {bottom_platform_roas:.2f})")
...
# ------------------ FILTER DATA ------------------
daily_filtered = daily[(daily['date'] >= pd.to_datetime(selected_dates[0])) &
                       (daily['date'] <= pd.to_datetime(selected_dates[1]))]
...
with tab3:
    ...
    gb = GridOptionsBuilder.from_dataframe(
        top_campaigns[['campaign', 'platform', 'ROAS', 'impression', 'clicks', 'spend', 'attributed revenue']]
    )
